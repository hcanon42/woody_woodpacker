...
# define CODE_OFFSET (text_seg->p_offset + text_seg->p_memsz)
...
{
	pack->_fd = open(to_read, O_APPEND | O_RDWR);
	if (pack->_fd == -1)
	{
		pack->_type = 0;
		return ;
...
int		inject_code(unsigned char *f_mmaped, t_packer *pack)
{
	...
	// 	return (-1);
	// }
	elf_pt_load = (Elf64_Phdr *) ((unsigned char *)elf_begin + (unsigned int)elf_begin->e_phoff);	/* going to beginning of segment */

	ft_putstr("[+] Finding a free space under a PT_LOAD segment...\n");
	for (int i = 0; i < elf_begin->e_phnum; i++) 	/* searching for PT_LOAD */
	{
		...
		printf("\nelf_pt_load->p_type = %d\n", elf_pt_load->p_type);
		printf("elf_pt_load->p_offset = %ld\n", elf_pt_load->p_offset);
		printf("elf_pt_load->p_memsz = %ld\n", elf_pt_load->p_memsz);
		printf("elf_pt_load->p_vaddr = 0x%lx\n", elf_pt_load->p_vaddr);
		printf("elf_pt_load->p_paddr = 0x%lx\n", elf_pt_load->p_paddr);
	...
	// 	return (-1);
	// }
	ft_printf("[+] Free space found at 0x%x: %d bytes.\n", text_seg->p_offset, gap);

	ft_printf("[+] Overwriting entry point (0x%.8x) program with shellcode adress (0x%.8x)...\n", old_entry, CODE_ADRESS);
	elf_begin->e_entry = (old_entry + text_seg->p_offset + text_seg->p_memsz);	/* changing old entry of the code by the address of ours */
	...
	ft_printf("[+] Injecting code (%d bytes) at offset %.8x (virtual adress 0x%.8x)...\n", code_size, CODE_OFFSET, CODE_ADRESS);
	insert_code(f_mmaped + CODE_OFFSET);

	ft_putstr("[+] Updating segment size...\n");
	text_seg->p_memsz += code_size;
	text_seg->p_filesz += code_size;
	return (0);
}
